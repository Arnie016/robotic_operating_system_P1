...
Member A (Matric: \texttt{[ID]}) focused on planner implementation, path post-processing, and planner parameter tuning. Member B (Matric: \texttt{[ID]}) focused on regulated pure pursuit implementation, lookahead and velocity regulation tuning, and final-yaw behavior. Member C (Matric: \texttt{[ID]}) focused on experiment execution, logging, evidence curation, and report integration. The final submission should replace this template with exact per-member ownership and verified contributions.
